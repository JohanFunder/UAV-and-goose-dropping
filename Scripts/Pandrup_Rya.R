# Indlæs udtyndet csv
directory <- "G:/Projekter/Pandrup_RYAE_UTM10m/Thin_photos"

Pandrup_RYAE_UTM10m <- file.path(directory,"Pandrup_Rya_UTM10m.csv")
Pandrup_RYAE_UTM10m
Pandrup_R_UTM10m <- read.csv(file=Pandrup_RYAE_UTM10m, header= TRUE)
str(Pandrup_R_UTM10m)


# Read udvalgte
udvalgte <- Pandrup_R_UTM10m
...
current.folder <- "G:/Overflyvninger_lave/april_14_Pandrup"

New <- dir.create("G:/Projekter/Pandrup_RYAE_UTM10m/Thin")
new.folder <- ("New")
...
file.copy(file.path("G:/Overflyvninger_lave/april_14_Pandrup",
                    list.of.files), "G:/Projekter/Pandrup_RYAE_UTM10m/Thin", copy.mode = TRUE)
...
system2("exiftool", args="-RelativeAltitude -GPSlatitude -GPSlongitude -GimbalYawDegree -ImageHeight -ImageWidth 
        ...
        G:/Projekter/Pandrup_RYAE_UTM10m/Thin",
        stdout="G:/Projekter/Pandrup_RYAE_UTM10m/Pandrup_R_photodata.csv",stderr = TRUE)

# Indlæs .csv
data1 <- read.csv("G:/Projekter/Pandrup_RYAE_UTM10m/Pandrup_R_photodata.csv")
View(data1)

# Indsæt GPS koord dec og UTM.
GPS_UTM <- read.csv("G:/Projekter/Pandrup_RYAE_UTM10m/Thin_photos/Pandrup_Rya_UTM10m.csv")[ ,c('xcoord', 'ycoord')]
...
metadata$cm.pr.pixel <- ((metadata$sensor.width.of.the.camera.mm)*(metadata$RelativeAltitude*100))/
  ((metadata$focal.length.of.the.camera.mm)*(ImageWidth))
...
# skriv .csv
write.csv(metadata, file = "G:/Projekter/Pandrup_RYAE_UTM10m/metadata_areal.csv")

View(read.csv("G:/Projekter/Pandrup_RYAE_UTM10m/metadata_areal.csv"))
...
write_worldfile <- function(Northing, Easting,angle,
  ...
  for (i in 1:length(filename.ext)){
    ...
    write.table(c(A[i],D[i],B[i],E[i],C[i],Fa[i]),
                paste('G:/Projekter/Pandrup_RYAE_UTM10m/Thin',cufile,sep = '/'), sep = '\n', row.names = FALSE
                ,col.names = FALSE)
  }
}
...
dat_GPS <- read.table('G:/Projekter/Pandrup_RYAE_UTM10m/Thin_photos/Pandrup_Rya_UTM10m.csv', header = TRUE, sep = ',', dec ='.')
dat_drone <- read.table('G:/Projekter/Pandrup_RYAE_UTM10m/metadata_areal.csv', header = TRUE, sep = ',', dec ='.')
dat_drone
