# Indlæs udtyndet csv
directory <- "H:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_points"

Nørholm_V_10m <- file.path(directory,"Noerholm_V_UTM10m_ny.csv")
Nørholm_V_10m
N_V_10m <- read.csv(file=Nørholm_V_10m, header= TRUE)
str(N_V_10m)
...
current.folder <- "H:/P8_projekt/Overflyvninger_lave/april_12_Nørholm_vest_lav_overflyvning"

New <- dir.create("H:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_ny")
new.folder <- ("New")
...
file.copy(file.path("H:/P8_projekt/Overflyvninger_lave/april_12_Nørholm_vest_lav_overflyvning",
                    list.of.files), "H:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_ny", copy.mode = TRUE)
...
list.of.files_JGW
# copy the files to the new folder
file.copy(file.path("H:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_5m",
                    list.of.files_JGW), "H:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_ny", copy.mode = TRUE)
...
system2("exiftool", args="-RelativeAltitude -GPSlatitude -GPSlongitude -GimbalYawDegree -ImageHeight -ImageWidth 
        ...
        H:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin__ny",
        stdout="H:/P8_projekt/Projekter/Noerholm_V_UTM10m/N_V_photodata_10m_ny.csv",stderr = TRUE)

# Indlæs .csv
data1 <- read.csv("H:/P8_projekt/Projekter/Noerholm_V_UTM10m/N_V_photodata_10m_ny.csv")
View(data1)

# Indsæt GPS koord dec og UTM.
GPS_UTM <- read.csv("G:/Projekter/Noerholm_V_UTM10m/Noerholm_points/Noerholm_UTM10meter.csv")[ ,c('xcoord', 'ycoord')]
...
# skriv .csv
write.csv(metadata, file = "G:/Projekter/Noerholm_V_UTM10m/metadata_areal.csv")
...
write_worldfile <- function(Northing, Easting,angle,
  ...
  for (i in 1:length(filename.ext)){
    ...
    write.table(c(A[i],D[i],B[i],E[i],C[i],Fa[i]),
                paste('G:/Projekter/Noerholm_V_UTM10m/Thin',cufile,sep = '/'), sep = '\n', row.names = FALSE
                ,col.names = FALSE)
  }
}
...
dat_GPS <- read.table('G:/Projekter/Noerholm_V_UTM10m/Noerholm_points/Noerholm_UTM10meter.csv', header = TRUE, sep = ',', dec ='.')
dat_drone <- read.table('G:/Projekter/Noerholm_V_UTM10m/metadata_areal.csv', header = TRUE, sep = ',', dec ='.')
