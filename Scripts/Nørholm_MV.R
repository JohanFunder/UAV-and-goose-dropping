# Indlæs udtyndet csv
directory <- "G:/Projekter/Noerholm_MV_UTM10m/Thin_photos"

Nørholm_MV_10m <- file.path(directory,"N_MV_10m.csv")
Nørholm_MV_10m
N_MV_10m <- read.csv(file=Nørholm_MV_10m, header= TRUE)
str(N_MV_10m)
...
current.folder <- "G:/Overflyvninger_lave/april_01_Nørholm_tætpåvej"

New <- dir.create("G:/Projekter/Noerholm_MV_UTM10m/Thin")
new.folder <- ("New")
...
file.copy(file.path("G:/Overflyvninger_lave/april_01_Nørholm_tætpåvej",
                    list.of.files), "G:/Projekter/Noerholm_MV_UTM10m/Thin", copy.mode = TRUE)
...
system2("exiftool", args="-RelativeAltitude -GPSlatitude -GPSlongitude -GimbalYawDegree -ImageHeight -ImageWidth 
        ...
        G:/Projekter/Noerholm_MV_UTM10m/Thin",
        stdout="G:/Projekter/Noerholm_MV_UTM10m/N_M_photodata.csv",stderr = TRUE)

# Indlæs .csv
data1 <- read.csv("G:/Projekter/Noerholm_MV_UTM10m/N_M_photodata.csv")
View(data1)

# Indsæt GPS koord dec og UTM.
GPS_UTM <- read.csv("G:/Projekter/Noerholm_MV_UTM10m/Thin_photos/N_MV_10m.csv")[ ,c('xcoord', 'ycoord')]
...
# skriv .csv
write.csv(metadata, file = "G:/Projekter/Noerholm_MV_UTM10m/metadata_areal.csv")
...
write_worldfile <- function(Northing, Easting,angle,
  ...
  for (i in 1:length(filename.ext)){
    ...
    write.table(c(A[i],D[i],B[i],E[i],C[i],Fa[i]),
                paste('G:/Projekter/Noerholm_MV_UTM10m/Thin',cufile,sep = '/'), sep = '\n', row.names = FALSE
                ,col.names = FALSE)
  }
}
...
dat_GPS <- read.table('G:/Projekter/Noerholm_MV_UTM10m/Thin_photos/N_MV_10m.csv', header = TRUE, sep = ',', dec ='.')
dat_drone <- read.table('G:/Projekter/Noerholm_MV_UTM10m/metadata_areal.csv', header = TRUE, sep = ',', dec ='.')
