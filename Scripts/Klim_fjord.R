# Indlæs udtyndet csv
directory <- "G:/Projekter/Klim_fjordholme_UTM10m/Thin_photos"

Klim_fjord_10m <- file.path(directory,"K_F_10m.csv")
Klim_fjord_10m
K_F_10m <- read.csv(file=K_F_10m, header= TRUE)
str(K_F_10m)
...
current.folder <- "G:/Overflyvninger_lave/april_11_Klim_fjordholme_(grusvej)"

New <- dir.create("G:/Projekter/Klim_fjordholme_UTM10m/Thin")
new.folder <- ("New")
...
file.copy(file.path("G:/Overflyvninger_lave/april_11_Klim_fjordholme_(grusvej)",
                    list.of.files), "G:/Projekter/Klim_fjordholme_UTM10m/Thin", copy.mode = TRUE)
...
system2("exiftool", args="-RelativeAltitude -GPSlatitude -GPSlongitude -GimbalYawDegree -ImageHeight -ImageWidth 
        ...
        G:/Projekter/Klim_fjordholme_UTM10m/Thin",
        stdout="G:/Projekter/Klim_fjordholme_UTM10m/K_F_photodata.csv",stderr = TRUE)

# Indlæs .csv
data1 <- read.csv("G:/Projekter/Klim_fjordholme_UTM10m/K_F_photodata.csv")
View(data1)

# Indsæt GPS koord dec og UTM.
GPS_UTM <- read.csv("G:/Projekter/Klim_fjordholme_UTM10m/Thin_photos/K_F_10m.csv")[ ,c('xcoord', 'ycoord')]
...
# skriv .csv
write.csv(metadata, file = "G:/Projekter/Klim_fjordholme_UTM10m/metadata_areal.csv")
...
write_worldfile <- function(Northing, Easting,angle,
  ...
  for (i in 1:length(filename.ext)){
    ...
    write.table(c(A[i],D[i],B[i],E[i],C[i],Fa[i]),
                paste('G:/Projekter/Klim_fjordholme_UTM10m/Thin',cufile,sep = '/'), sep = '\n', row.names = FALSE
                ,col.names = FALSE)
  }
}
...
dat_GPS <- read.table('G:/Projekter/Klim_fjordholme_UTM10m/Thin_photos/K_F_10m.csv', header = TRUE, sep = ',', dec ='.')
dat_drone <- read.table('G:/Projekter/Klim_fjordholme_UTM10m/metadata_areal.csv', header = TRUE, sep = ',', dec ='.')
