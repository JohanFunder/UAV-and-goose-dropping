# Indlæs udtyndet csv
directory <- "G:/P8_projekt/Projekter/Noerholm_V_UTM10m/Noerholm_V_5m"

Nørholm_V_5m <- file.path(directory,"Noerholm_V_5m_photodata.csv")
Nørholm_V_5m
N_V_5m <- read.csv(file=Nørholm_V_5m, header= TRUE)
str(N_V_5m)
...
current.folder <- "G:/P8_projekt/Overflyvninger_lave/april_12_Nørholm_vest_lav_overflyvning"

New <- dir.create("G:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_5m")
new.folder <- ("New")
...
file.copy(file.path("G:/P8_projekt/Overflyvninger_lave/april_12_Nørholm_vest_lav_overflyvning",
                    list.of.files), "G:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_5m", copy.mode = TRUE)
...
system2("exiftool", args="-RelativeAltitude -GPSlatitude -GPSlongitude -GimbalYawDegree -ImageHeight -ImageWidth 
        ...
        G:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_5m",
        stdout="G:/P8_projekt/Projekter/Noerholm_V_UTM10m/N_V_photodata_5m.csv",stderr = TRUE)

# Indlæs .csv
data1 <- read.csv("G:/P8_projekt/Projekter/Noerholm_V_UTM10m/N_V_photodata_5m.csv")
View(data1)

# Indsæt GPS koord dec og UTM.
GPS_UTM <- read.csv("G:/P8_projekt/Projekter/Noerholm_V_UTM10m/Noerholm_V_5m/Noerholm_V_5m_photodata.csv")[ ,c('xcoord', 'ycoord')]
...
##OBS: Bevar PHANTOM 4 PRO i dette tilfælde
a <- c(8.6)
b <- c(12.833)
...
metadata$cm.pr.pixel <- ((metadata$sensor.width.of.the.camera.mm)*(metadata$RelativeAltitude*100))/
  ((metadata$focal.length.of.the.camera.mm)*(metadata$ImageWidth))
...
# skriv .csv
write.csv(metadata, file = "G:/P8_projekt/Projekter/Noerholm_V_UTM10m/metadata_areal_5m.csv")
...
write_worldfile <- function(Northing, Easting,angle,
  ...
  for (i in 1:length(filename.ext)){
    ...
    write.table(c(A[i],D[i],B[i],E[i],C[i],Fa[i]),
                paste('G:/P8_projekt/Projekter/Noerholm_V_UTM10m/Thin_5m',cufile,sep = '/'), sep = '\n', row.names = FALSE
                ,col.names = FALSE)
  }
}
...
dat_GPS <- read.table('G:/P8_projekt/Projekter/Noerholm_V_UTM10m//Noerholm_V_5m/Noerholm_V_5m_photodata.csv', header = TRUE, sep = ',', dec ='.')
dat_drone <- read.table('G:/P8_projekt/Projekter/Noerholm_V_UTM10m/metadata_areal_5m.csv', header = TRUE, sep = ',', dec ='.')
