# Indlæs udtyndet csv
directory <- "G:/P8_projekt/Projekter/Krap_syd_UTM10m/Thin_photos"

Krap_syd_10m <- file.path(directory,"Krap_syd_10m.csv")
Krap_syd_10m
Krap_s_10m <- read.csv(file=Krap_syd_10m, header= TRUE)
str(Krap_s_10m)
...
current.folder <- "G:/P8_projekt/Overflyvninger_lave/marts_31_Kraptårn_syd"

New <- dir.create("G:/P8_projekt/Projekter/Krap_syd_UTM10m/Thin")
new.folder <- ("New")
...
file.copy(file.path("G:/P8_projekt/Overflyvninger_lave/marts_31_Kraptårn_syd",
                    list.of.files), "G:/Projekter/Krap_syd_UTM10m/Thin", copy.mode = TRUE)
...
system2("exiftool", args="-RelativeAltitude -GPSlatitude -GPSlongitude -GimbalYawDegree -ImageHeight -ImageWidth 
        ...
        G:/P8_projekt/Projekter/Krap_syd_UTM10m/Thin",
        stdout="G:/P8_projekt/Projekter/Krap_syd_UTM10m/Krap_S_photodata.csv",stderr = TRUE)

# Indlæs .csv
data1 <- read.csv("G:/Projekter/Krap_syd_UTM10m/Krap_S_photodata.csv")
View(data1)

# Indsæt GPS koord dec og UTM.
GPS_UTM <- read.csv("G:/Projekter/Krap_syd_UTM10m/Thin_photos/Krap_Syd_10m.csv")[ ,c('xcoord', 'ycoord')]
...
# skriv .csv
write.csv(metadata, file = "G:/Projekter/Krap_syd_UTM10m/metadata_areal.csv")
...
write_worldfile <- function(Northing, Easting,angle,
  ...
  for (i in 1:length(filename.ext)){
    ...
    write.table(c(A[i],D[i],B[i],E[i],C[i],Fa[i]),
                paste('G:/Projekter/Krap_syd_UTM10m/Thin',cufile,sep = '/'), sep = '\n', row.names = FALSE
                ,col.names = FALSE)
  }
}
...
dat_GPS <- read.table('G:/Projekter/Krap_syd_UTM10m/Thin_photos/Krap_Syd_10m.csv', header = TRUE, sep = ',', dec ='.')
dat_drone <- read.table('G:/Projekter/Krap_syd_UTM10m/metadata_areal.csv', header = TRUE, sep = ',', dec ='.')
