# Indlæs udtyndet csv
directory <- "G:/Projekter/Noerholm_E_S_UTM10m/All_E_S_Thin"

Nørholm_E_S_10m <- file.path(directory,"N_E_S_10m.csv")
Nørholm_E_S_10m
N_E_S_10m <- read.csv(file=Nørholm_E_S_10m, header= TRUE)
str(N_E_S_10m)
...
New <- dir.create("G:/Projekter/Noerholm_E_S_UTM10m/Thin")
new.folder <- ("New")
...
file.copy(file.path("G:/Overflyvninger_lave/marts_28_Nørholm_østlige_mark(28_3)",
                    list.of.files), "G:/Projekter/Noerholm_E_S_UTM10m/Thin", copy.mode = TRUE)
...
system2("exiftool", args="-RelativeAltitude -GPSlatitude -GPSlongitude -GimbalYawDegree -ImageHeight -ImageWidth 
        ...
        G:/Projekter/Noerholm_E_S_UTM10m/Thin",
        stdout="G:/Projekter/Noerholm_E_S_UTM10m/N_E_S_photodata.csv",stderr = TRUE)

# Indlæs .csv
data1 <- read.csv("G:/Projekter/Noerholm_E_S_UTM10m/N_E_S_photodata.csv")
View(data1)

# Indsæt GPS koord dec og UTM.
GPS_UTM <- read.csv("G:/Projekter/Noerholm_E_S_UTM10m/All_E_S_Thin/N_E_S_10m.csv")[ ,c('xcoord', 'ycoord')]
...
# skriv .csv
write.csv(metadata, file = "G:/Projekter/Noerholm_E_S_UTM10m/metadata_areal.csv")
...
write_worldfile <- function(Northing, Easting,angle,
  ...
  for (i in 1:length(filename.ext)){
    ...
    write.table(c(A[i],D[i],B[i],E[i],C[i],Fa[i]),
                paste('G:/Projekter/Noerholm_E_S_UTM10m/Thin',cufile,sep = '/'), sep = '\n', row.names = FALSE
                ,col.names = FALSE)
  }
}
...
dat_GPS <- read.table('G:/Projekter/Noerholm_E_S_UTM10m/All_E_S_Thin/N_E_S_10m.csv', header = TRUE, sep = ',', dec ='.')
dat_drone <- read.table('G:/Projekter/Noerholm_E_S_UTM10m/metadata_areal.csv', header = TRUE, sep = ',', dec ='.')
